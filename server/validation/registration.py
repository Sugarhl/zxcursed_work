...
async def check_user_exists(
    ...
    existing_user = await get_user_by_email(
        db, email=user_in.email, user_type=user_type
    )
    if existing_user:
        raise HTTPException(
...
def validate_user_data(user_type: UserType, user_in: UserIn):
    ...
    if not user_in.first_name or len(user_in.first_name) > 50:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid first name"
        )
    if not user_in.last_name or len(user_in.last_name) > 50:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid last name"
        )
